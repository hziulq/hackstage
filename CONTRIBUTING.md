...
PR には Lint / テスト / `openapi.json` 生成差分の結果が表示される（憲章「CI」節）。
**必須ステータスチェックではないため、CI が赤くてもマージ自体は可能。**
経験の浅いメンバーが CI に阻まれて作業を止めないようにするための運用。

- CI が赤い場合でも、まずは内容を確認し、直せる範囲で直す。直せない/わからない場合は
  そのままレビュー依頼してよい（レビュアーに CI の結果が見えている前提で判断してもらう）。
- ワークフロー定義は `.github/workflows/`（`007-ci-setup` で追加）。
